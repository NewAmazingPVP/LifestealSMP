    ...
    public void playerInteract(PlayerInteractEvent event){
        ...
        if ((event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) && event.hasItem() && event.getItem().getType() == Material.NETHERITE_SHOVEL) {
            ...
            if (meta.getLore().toString().contains("Right click to throw the land")) {
                ...
                CooldownManager cooldown = tarraflingerCooldowns.get(attacker);
                if (cooldown == null) {
                    cooldown = new CooldownManager();
                    tarraflingerCooldowns.put(attacker, cooldown);
                    cooldown = tarraflingerCooldowns.get(attacker);
                }

                if (!cooldown.isOnCooldown()) {
                    getServer().getScheduler().runTaskLater(lifestealSmp, () -> event.getPlayer().setCooldown(itemInMainHand.getType(), tarraflingerCooldownTime * 20), 1);
                    tarraflingerCooldowns.put(attacker, cooldown);
                    cooldown.setCooldown(tarraflingerCooldownTime);
